                ...
                >
                    ...
                    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                        ...
                        <div className="mt-8">
                            ...
                            <ul role="list" className="-my-6 divide-y divide-gray-200">
                                ...
                                <li key={product.id + Math.random()} className="flex py-6">
                                    ...
                                    <div>
                                        ...
                                        <h3>
                                            <Link legacyBehavior href={`/products/${product.handle}`}>
                                                <a onClick={() => setCartOpen(false)}>{product.title}</a>
                                            </Link>
                                        </h3>
                                        <p className="ml-4">{formmatter.format(product.variantPrice)}</p>
                                        </div>
                            ...
                            { cart.length === 0 && <p>Nothing in your cart!</p>}
                            </div>
                        </div>
                        </div>
                        { cart.length ? 
                            <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                                <div className="flex justify-between text-base font-medium text-gray-900">
                                    <p>Subtotal</p>
                                    <p>{formmatter.format(cartTotal)}</p>
                                </div>
                                <p className="mt-0.5 text-sm text-gray-500">Shipping and taxes calculated at checkout.</p>
                                <div className="mt-6">
                                    <a
                                    href={checkoutUrl}
                                    className="flex items-center justify-center rounded-md border border-transparent bg-yellow-500 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-yellow-600"
                                    >
                                    Checkout
                                    </a>
                                </div>
